//start by declaring known vriables with starting values
//variables: data storage device; friendly name for info 
...
function shoot() {
    ...
    //conditional statment --> checks for TRUTH; uses relational (conditional) operaters
    if (currentAmmo > 0) {
    ...
    //once the if statment has run, update the screen 
    //call the updating screen fumction --> call by stating the name
...
//function can also be PASSED VALUES; these values are called PARAMETERS and must be sent to the function during the call in order for the function to work (func relies on this data to do its job!)
